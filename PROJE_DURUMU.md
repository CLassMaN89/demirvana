...
- `Sıralama Değişimi` paneli, yönetim panelinde daha önce kullanılan `Site Ziyaretçi İstatistikleri` alan grafiğiyle aynı mavi/yeşil dolgulu görsel yapıya geçirildi. Masaüstü ve mobil serileri ziyaret kayıtlarının kullanıcı ajanı ve tarihinden gerçek zamanlı gruplanır; lejant aynı renklerle gösterilir.
- `Sıralama Değişimi` grafiğindeki ilgisiz Masaüstü/Mobil serileri kaldırıldı. Grafik aynı alan-grafik görselini koruyarak gerçek canlı analizdeki Demir Vana, Duyar, FAF, Dikkaya ve Ayvaz sitelerini ayrı renkli seri olarak; Performans, SEO, Erişilebilirlik ve En İyi Uygulamalar puanları üzerinden gösterir.
