...
- İlk genel bakışın referansa göre fazla boş kaldığı kullanıcı geri bildirimiyle doğrulandı ve yalnız SEO Merkezi yeniden düzenlendi: son 30 gerçek denetim API yanıtına eklendi; sağlık puanı geçmişi çizgi grafiğine, son taramalar hareket listesine, gerçek sağlık/indekslenebilir URL değerleri karşılaştırma tablosuna ve ilk beş gerçek sorun alt görev şeridine bağlandı. `seo.png` sprite kırpmalarında taşma giderildi; SEO Merkezi, Google, Site Sağlığı ve Reklam ikonları kontrollü kutular içinde gösterilir. Rakip/sıralama/reklam dış verileri bağlantısızken sayı uydurulmaz.
- SEO rakip izleme altyapısı eklendi. Demir Vana, Duyar, FAF, Dikkaya ve Ayvaz'ın resmî ana adresleri `seo_rakipleri` tablosunda; her canlı ölçümün HTTP durumu, yanıt süresi, teknik SEO puanı, başlık/meta uzunluğu, H1, kelime, bağlantı, schema ve sitemap URL sayısı `seo_rakip_taramalari` tablosunda tarihsel olarak saklanır. Yerel canlı ölçümde sırasıyla Demir Vana 68/100 ve 500 sitemap kaydı, Duyar 88/100 ve 2, FAF 90/100 ve 1834, Dikkaya 50/100 ve 0, Ayvaz 90/100 ve 9 ölçüldü; bunlar sabit örnek değil, yenileme sırasında resmî sitelerden tekrar alınır. Karşılaştırma ve son rakip analizleri kartları bu gerçek kayıtlara bağlandı. `seo.png` ikon kutuları taşmayacak biçimde küçültülüp oranı korunarak görsel olarak yeniden denetlendi.
- Kullanıcının ikonların hâlâ referansla uyuşmadığı geri bildirimi üzerine önceki CSS arka plan kırpımları kaldırıldı. `SeoGorselIkonu.jsx`, 1536×1024 kaynak `seo.png` dosyasını her ikonun gerçek merkez koordinatına göre ölçekleyip sabit taşma penceresinde gösterir; böylece en-boy oranı bozulmaz ve ikon kenarları kesilmez. Ana SEO ikonu, yedi sekme ikonu, Google/sıralama/grafik/performans/sağlık KPI ikonları ve reklam ikonu tek sistemden gelir. KPI renkleri ve alt veri çizgileri referansın mavi/yeşil/mor/turuncu ayrımına yaklaştırıldı; canlı tarayıcı ekran görüntüsüyle tekrar kontrol edildi.
...
- Genel bakıştaki `Site Sağlığı Değişimi` paneli `Sıralama Değişimi` olarak değiştirildi. Grafik artık SEO tarama sağlık geçmişini değil, mevcut Site Ziyaretçi İstatistikleri servisindeki günlük görüntüleme sayılarını kullanır; tooltip ve panel kaynağı buna göre güncellendi.
- SEO Merkezi'ndeki ikincil/gezinme butonları proje `BUTON-STANDARDI.md` görünümüne eşlendi: soluk mavi zemin ve marka renkli ince kenarlık, hover durumunda dolu mavi zemin/beyaz metin ve sağ ok hareketi. Birincil `Siteyi tara/Yeniden tara` ile `Rapor İndir` eylemleri dolu görünümünü korur; yenile ikon butonu aynı standart hover davranışını kullanır.
- `Sıralama Değişimi` paneli, daha önce kullanılan `Site Ziyaretçi İstatistikleri` alan grafiğinin Recharts yapısıyla düzenlendi. İlgisiz Masaüstü/Mobil serileri kaldırıldı. API, `seo_rakip_taramalari` tablosundaki son 150 gerçek ölçümü `rakip_gecmisi` olarak döndürür; grafik Demir Vana ve rakip site adlarını seri, tarama zamanlarını X ekseni ve gerçek SEO puanlarını Y ekseni olarak kullanır.
- `Sıralama Değişimi` grafiğinin önceki çizgi grafik ayarları kaldırıldı ve son verilen gradient alan grafik promptu uygulandı. Recharts `AreaChart`, her gerçek site serisi için ayrı dikey gradient, `3 3` kesik ince çizgiler, kesik yatay grid ve imleç çizgisi olmayan tooltip kullanır; demo Desktop/Mobile verileri eklenmemiştir.
- `Bugün Ne Yapmalıyım?` şeridi referans görsele uyarlandı: hedef ikonu, gerçek sorunlardan üretilen beş numaralı ürün görevi, ölçülen karakter sayılarını gösteren önem renkli rozetler ve İçerik Fırsatları sekmesine giden `Detaylı Görev Listesi` butonu eklendi.
...
- SEO Merkezi yatay sekmeleri, sol menüye dokunulmadan onun lacivert zemin, açık pasif metin, beyaz hover, mavi yarı saydam aktif vurgu ve Lucide ikon diline uyarlandı. Yedi eşit sütun dar ekranlarda 4 ve 2 sütuna kırılır; yatay scroll oluşturmaz.
- Kullanıcı geri bildirimiyle SEO Merkezi yatay sekmeleri admin sol menü görünümünden çıkarılıp halka açık sitenin beyaz zemin, koyu metin, açık mavi hover ve tema mavisi aktif çizgi stiline geçirildi. Aktif çizgi sekmeler arasında kayar, sekme içeriği kısa bir geçişle görünür; azaltılmış hareket tercihinde animasyonlar kapanır. Sol menü değiştirilmedi ve yatay scroll oluşturulmadı.
- İlk yatay uyarlamadaki görünmeyen border ve eksik hover geçişi düzeltildi: halka açık ürün sol menüsündeki tek kayan vurgu elemanı SEO sekmelerine yatay olarak uyarlandı. Hover/focus sırasında `#edf5fe` şerit ve 3px `#3277fd` alt border öğeler arasında 240ms geçişle kayar; aktif sekme aynı görünümü kalıcı taşır. Mobil kırılımda border artık kapatılmaz, yatay scroll yoktur ve sekme içerik geçişi korunur.
- SEO yatay menüsünün hover ve aktif yazı/ikon rengi kullanıcı isteğiyle `#125abf` olarak sabitlendi.
- SEO Merkezi alt sekmeleri mevcut gerçek veri sözleşmesiyle dolduruldu: Rakip Intelligence son canlı rakip ölçümlerini karşılaştırmalı tabloda, İçerik Fırsatları gerçek tarama sorunlarını ve önerilen düzeltmeleri, Reklam Takibi Google Ads bağlantı durumunu, Raporlar site sağlığı/rakip ölçümlerini ve indirilebilir CSV çıktısını gösterir. Site Sağlığı mevcut çalışan tarama akışını korur. Anahtar Kelimeler sekmesi, sıralama/Search Console sağlayıcısı bağlı olmadığı için sahte metrik üretmeden bağlantı gereksinimini açıklar.
- Anahtar Kelimeler sekmesi kullanıcı kararıyla hesap bağlantısı gerektiren sıralama ekranından, herkese açık rakip içerik analizine çevrildi. Rakip taraması her sitenin ana sayfa metninden tekrar eden ilk 30 anlamlı terimi, tekrar sayısını ve başlık/H1 sinyalini çıkarıp `seo_rakip_taramalari.anahtar_kelimeler_json` alanında tarihsel ölçümle birlikte saklar. Sekme terimin geçtiği rakipleri, toplam tekrarı, SEO sinyalini ve Demir Vana kataloğuna göre içerik açığını gösterir; `Kelimeleri güncelle` mevcut canlı rakip taramasını yeniden çalıştırır. Üretim alan adı yerelde zaman aşımına uğrarsa Demir Vana karşılaştırması gerçek ürün ve kategori kayıtlarından beslenir; kesin Google sıralaması, CTR veya gösterim değeri üretilmez.
